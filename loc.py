...
sess.post('http://www.hostloc.com/member.php?mod=logging&action=login&loginsubmit=yes&infloat=yes&lssubmit=yes&inajax=1',headers=headers,data=data)
...
results = pool.map(lambda x: sess.get(x), [ 'http://www.hostloc.com/space-uid-{}.html'.format(random.randint(10000,20000)) for i in range(20) ]) # urls是任务列表 list，第一个参数是线程函数
